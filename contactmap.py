...
def contactmap_yang(selection='', chain='', align='hmm2.map', pcut=0.999, offset=0):
    '''
DESCRIPTION
...
ARGUMENTS
    ...
    align = string: name of map_align result txt file {default: 'hmm2.map'}
...
EXAMPLE
    ...
    contactmap_yang 5dir, A, /path/to/hmmrenumbered.map, 0.998, offset=-7
    ...
    with open(align) as f:
        ...
        for s in l:
            ...
            a = int(sec[0]) + int(offset)
            b = int(sec[1]) + int(offset)
            probability = float(sec[4])
...
cmd.extend('contactmap_yang', contactmap_yang)

# tab-completion of arguments
cmd.auto_arg[0].update({
    'contactmap_yang'       : cmd.auto_arg[0]['zoom'],
})
