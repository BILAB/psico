...
def fetch(code, name='', type='pdb', quiet=1, bsync=-1, **kwargs):
    '''
PSICO NOTES
    ...
    if type != 'pdb':
        return pymol_fetch(code, name, type=type, quiet=quiet, bsync=bsync, **kwargs)
    ...
    for code in list(code_list):
        if len(code) in [5,6]:
            ...
            if os.path.exists(filename):
                ...
                code_list.remove(code)
    if len(code_list) > 0:
        return pymol_fetch(' '.join(code_list), name, quiet=quiet, bsync=bsync, **kwargs)
...
def fetch_cath(code, name='', **kwargs):
    if name == '':
        name = code
    r = fetch(code[:4], name, bsync=0, **kwargs)
    if cmd.is_error(r):
        return r
    if code[5:7] == '00':
        if code[4] != '0':
...
def fetch_scop(code, name='', **kwargs):
    if name == '':
        name = code
    r = fetch(code[1:5], name, bsync=0, **kwargs)
    if cmd.is_error(r):
        return r
    try:
        import Bio.SCOP
