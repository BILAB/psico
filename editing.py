...
SEE ALSO
    ...
    for model in cmd.get_object_list(selection):
        ...
        for line in process.stdout:
            if line.startswith(b'  #  RESIDUE'):
                break
        for line in process.stdout:
            resi = line[5:11].strip().decode()
            chain = line[11:12].strip().decode()
            ss = line[16:17].decode()
    ...
    for model in cmd.get_object_list(selection):
        ...
        for line in process.stdout:
            if not line.startswith('ASG'):
                continue
            chain = line[9:10].strip('-').decode()
            resi = line[11:16].strip().decode()
            ss = line[24:25].decode()
            ss_dict[model,chain,resi] = ss
    os.remove(tmpfilepdb)
    _common_ss_alter(selection, ss_dict, ss_map, raw)
        ...
        for model in cmd.get_object_list('(' + selection + ')'):
            ...
            with open(tmpfilesst) as handle:
                ...
                for line in handle:
                    if not line.strip():
                        break
                    chain = line[6].strip('-').decode()
                    resi = line[7:12].strip().decode()
                    ss = line[23].decode()
                    ss_dict[model,chain,resi] = ss
    ...
    for model in cmd.get_object_list(selection):
        ...
        for line in lines:
            if line.startswith('...................................END'):
                break
            chain = line[2:3].strip().decode()
            resi = line[3:9].strip().decode()
            ss = line[21:22].decode()
            ss_dict[model,chain,resi] = ss
