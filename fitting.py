...
def alignwithanymethod(mobile, target, methods=None, bsync=1, quiet=1):
    '''
DESCRIPTION
...
ARGUMENTS
    ...
    bsync, quiet = int(bsync), int(quiet)
...
def promix(mobile, target, K=0, prefix=None, mobile_state=-1, target_state=-1,
        match='align', guide=1, quiet=1, bsync=-1, _self=cmd):
    '''
DESCRIPTION
...
REFERENCE
    ...
    guide, quiet, bsync = int(guide), int(quiet), int(bsync)
    if bsync < 0:
        bsync = not quiet
